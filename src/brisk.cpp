...
GstFlowReturn new_sample_callback(GstElement *sink, gpointer data) {
    Ptr<BRISK> *brisk = static_cast<Ptr<BRISK>*>(data);
    ...
    Mat descriptors;
    (*brisk)->detectAndCompute(gray, Mat(), keypoints, descriptors);
    ...
    // Display result
    imshow("BRISK Features - GStreamer", frame_with_keypoints);
    waitKey(1);
...
int main(int argc, char *argv[]) {
    ...
    GstElement *pipeline = gst_parse_launch(
        "filesrc location=/home/sebas/source/cs-26-sw-6-03/AAUSAT6-C-/src/samples/test_clip.mp4 ! "
        "decodebin ! "
        "videoconvert ! "
    ...
    g_signal_connect(appsink, "new-sample", 
                     G_CALLBACK(new_sample_callback), &brisk);
